...
theorem linear_rnn_affine_on_binary (C : Matrix (Fin 1) (Fin n) ℝ) (A : Matrix (Fin n) (Fin n) ℝ)
    ...
    ∀ (x y : ℝ), (x = 0 ∨ x = 1) → (y = 0 ∨ y = 1) →
      let inputs : Fin 2 → (Fin 1 → ℝ) := ![fun _ => x, fun _ => y]
      (C.mulVec (stateFromZero A B 2 inputs)) 0 = w₀ * x + w₁ * y + c := by
  -- The output function f(inputs) = (C * state(inputs))_0 is linear in inputs.
  -- By state_additive: f(x+y) = f(x) + f(y)
  -- By state_scalar: f(c*x) = c*f(x)
  -- So f(x,y) = x*f(1,0) + y*f(0,1) + 0
  -- where (x,y) means input sequence [x, y]
  sorry
...
theorem linear_cannot_xor :
  ...
  use w₀, w₁, c
  intro x y hx hy
  have h := h_f ![fun _ => x, fun _ => y]
  have h' := h_affine x y hx hy
  -- h says: (fun _ => xorReal x y) = C.mulVec (stateFromZero A B 2 inputs)
  -- Evaluate at 0
  have h_eq : xorReal x y = (C.mulVec (stateFromZero A B 2 ![fun _ => x, fun _ => y])) 0 := by
    have := congrFun h 0
