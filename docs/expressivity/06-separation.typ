...
#theorem("Strict Computational Hierarchy")[
  $ "Linear RNN" subset.neq "D-layer Linear-Temporal" subset.neq "E88" subset.neq "E23" $#leanfile("MultiLayerLimitations.lean:365")
]
...
#theorem("Affine Barrier")[
  Linear-temporal outputs are affine. XOR is not: $"XOR"(0,0) + "XOR"(1,1) = 0 eq.not 2 = "XOR"(0,1) + "XOR"(1,0)$.#leanfile("LinearLimitations.lean:218")
]

E88 escapes through sign encoding. Parity is stored in whether $S$ is positive or negative.

#theorem("Continuity Barrier")[
  Linear-temporal outputs are continuous. Threshold is discontinuous.#leanfile("ExactCounting.lean:344")
]
...
#theorem("E88 Computes Parity")[
  With $alpha = 1$ and $delta = 2$: input 0 preserves the sign of $S$; input 1 flips the sign.#leanfile("TanhSaturation.lean:720")
]

Input 0 gives $S' = tanh(S)$. Input 1 gives $S' = tanh(S + 2)$, which crosses zero when $S < 0$.

#theorem("E88 Computes Soft Threshold")[
  Accumulated positive inputs drive $S$ toward $+1$. Accumulated negative inputs drive $S$ toward $-1$.#leanfile("TanhSaturation.lean:424")
]
